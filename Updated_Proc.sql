create or replace procedure customer_signUp(
...
create or replace PROCEDURE deliveryOrder_setup(  
    c_id customer.id%type,
...
END;
/
create or replace PROCEDURE dineInOrder_setup(  
...
create or replace PROCEDURE member_updateByMemberId(   
    m_id member.id%type,
    m_discount member.discount%type,
    m_flavor member.flavor%type, 
    m_birthday member.birthday%type, 
...
create or replace PROCEDURE PAYMENT_SETUP(  
    o_type varchar2,
...
create or replace PROCEDURE reservation_setup(  
    c_id customer.id%type,
    t_id diningtable.id%type,
...
create or replace PROCEDURE customized_menu_setup(  
        m_id member.id%type
    )  
IS 
